# -*- coding: utf-8 -*-
"""Modelo de porteros (GK) — K-Means, k=3, 5 variables.

Ejes: carga de trabajo (SoTA/90), resolución de esa carga (Save%, CS%) y presencia
en el área (PKatt/90, Fld/90). Es el único modelo con umbral de entrenamiento de 15
partidos: sus variables son porcentajes, y un porcentaje sobre pocos partidos es muy
inestable. También el único que limita la búsqueda de k a 6, porque con 103 jugadores
de entrenamiento un k mayor produce clusters de tamaño irrelevante.

Resultado de referencia: silueta 0.188 (103 de entrenamiento) · 0.171 (137 etiquetados).

    python -m models.modelo_gk
"""
...
def main(datasets=None, graficar=True, guardar=True):
    """Ejecuta las fases 3, 4, 5 y 6 para porteros. Devuelve el dict de resultados."""
    if datasets is None:
    ...
    if graficar:
        ...
        k_ref = visualizacion.elegir_k(
            X, titulo=P.NOMBRE_POS[POS], entrena=prep["entrena"], k_max=P.K_MAX_GK,
